    ...
    bmak = bmak || {
        ...
        gd: function () {
            ...
            var d = Math.random(),
                k = bmak.pi(1e3 * d / 2),
                s = d + '';
            return s = s.slice(0, 11) + k, bmak.gbrv(), bmak.get_browser(), bmak.bc(), bmak.bmisc(), t + ',uaend,' + bmak.xagg + ',' + bmak.psub + ',' + bmak.lang + ',' + bmak.prod + ',' + bmak.plen + ',' + bmak.pen + ',' + bmak.wen + ',' + bmak.den + ',' + bmak.z1 + ',' + bmak.d3 + ',' + n + ',' + o + ',' + m + ',' + r + ',' + c + ',' + i + ',' + b + ',' + bmak.bd() + ',' + a + ',' + s + ',' + e + ',' + bmak.brv + ',loc:' + bmak.loc
        },
        ...
        bpd: function () {
            ...
            try {
                ...
                var k = bmak.ke_vel + bmak.me_vel + bmak.doe_vel + bmak.dme_vel + bmak.te_vel + bmak.pe_vel,
                    ...
                    f = bmak.pi(bmak.d2 / 6),
                    p = bmak.fas(),
                    h = [bmak.ke_vel + 1, bmak.me_vel + 32, bmak.te_vel + 32, bmak.doe_vel, bmak.dme_vel, bmak.pe_vel, k, a, bmak.init_time, bmak.start_ts, bmak.fpcf.td, bmak.d2, bmak.ke_cnt, bmak.me_cnt, f, bmak.pe_cnt, bmak.te_cnt, _, bmak.ta, bmak.n_ck, e, bmak.ab(e), bmak.fpcf.rVal, bmak.fpcf.rCFP, p, l, u[0], u[1]],
                    v = h.join(','),
            ...
            try {
                ...
                bmak.sensor_data = F + ';' + (bmak.get_cf_date() - t) + ';' + bmak.tst + ';' + (bmak.get_cf_date() - B)
            } catch (t) {}
            bmak.sd_debug('</bpd>')
        },
        od: function (t, a) {
        ...
        mn_s: function (t) {
            ...
            for (var h = 0; h < l; h++) {
                for (var v, g = new Array(64), w = e, y = n, E = o, S = m, C = r, v = i, M = c, x = b, _ = 0; _ < 64; _++) {
                    var j, A, L, P, T, D;
                    _ < 16 ? g[_] = u[h][_] : (j = bmak.rotate_right(g[_ - 15], 7) ^ bmak.rotate_right(g[_ - 15], 18) ^ g[_ - 15] >>> 3, A = bmak.rotate_right(g[_ - 2], 17) ^ bmak.rotate_right(g[_ - 2], 19) ^ g[_ - 2] >>> 10, g[_] = g[_ - 16] + j + g[_ - 7] + A), A = bmak.rotate_right(C, 6) ^ bmak.rotate_right(C, 11) ^ bmak.rotate_right(C, 25), L = C & v ^ ~C & M, P = x + A + L + a[_] + g[_], j = bmak.rotate_right(w, 2) ^ bmak.rotate_right(w, 13) ^ bmak.rotate_right(w, 22), T = w & y ^ w & E ^ y & E, D = j + T, x = M, M = v, v = C, C = S + P >>> 0, S = E, E = y, y = w, w = P + D >>> 0
                }
